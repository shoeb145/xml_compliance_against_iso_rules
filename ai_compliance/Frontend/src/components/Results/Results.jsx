...
const Results = ({ results, onReset, taskId, setResults }) => {
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      ...
      // Load logo for first page only
      let logoDataUrl = null;
      try {
        const logoResponse = await fetch("/frontend/macksofy_white.png");
      ...
      if (logoDataUrl) {
        ...
        // Fallback: Text logo in top-right
        pdf.setTextColor(255, 255, 255);
        pdf.setFontSize(10);
        pdf.setFont(undefined, "bold");
        pdf.text("Macksofy", pageWidth - 40, 20);
        pdf.setFontSize(8);
        pdf.setFont(undefined, "normal");
        pdf.text("TECHNOLOGIES", pageWidth - 40, 26);
      }
      ...
      allControls.forEach((rule, index) => {
        // Check if we need a new page (with space for footer)
        if (yPosition > 250) {
          pdf.addPage();
          yPosition = 20; // No header on subsequent pages
        }
      ...
      for (let i = 1; i <= totalPages; i++) {
        ...
        pdf.text(
          "Generated by Macksofy Technologies Compliance Checker",
          pageWidth / 2,
          295,
          {
            align: "center",
